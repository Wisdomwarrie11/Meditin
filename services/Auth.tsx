    ...
    <div className="min-h-screen flex bg-white selection:bg-brandOrange selection:text-white relative">
      ...
      {showVerificationModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-navy/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-10 sm:p-14 text-center space-y-8 shadow-2xl animate-in zoom-in-95 duration-500">
            <div className="w-24 h-24 bg-emerald-500 text-white rounded-[2.5rem] flex items-center justify-center mx-auto shadow-xl shadow-emerald-500/20">
              <Check size={48} strokeWidth={3} />
            </div>
            <div className="space-y-4">
              <h3 className="text-3xl font-black text-navy leading-none tracking-tighter">Check your email!</h3>
              <p className="text-slate-500 font-medium leading-relaxed">
                We've sent a verification link to <span className="text-navy font-bold">{formData.email}</span>. Please verify your email to access all features.
              </p>
            </div>
            <button 
              onClick={() => navigate('/dashboard')}
              className="w-full py-6 bg-navy text-white rounded-full font-black uppercase tracking-widest hover:bg-brandOrange transition-all shadow-xl active:scale-95"
            >
              Go to Dashboard
            </button>
          </div>
